                    ...
                    <table class="member-place-list owner-booking table-responsive">
                        ...
                        @foreach($reserves as $reserve)
                            <tr>
                                ...
                                <td data-title="Places"><b>{{$reserve->getBasePlace()->name}}</b></td>
                                <td data-title="Booking at">{{$reserve->reservation_date}}</td>
                                <td data-title="Booking at">{{$reserve->arrival_date}}</td>
                                <td data-title="Status" class=" @if($reserve->getBookingStatus()->id == 1) pending @elseif($reserve->getBookingStatus()->id == 2) approved @else cancel  @endif">{{$reserve->getBookingStatus()->name}}</td>
                                <td data-title="" class="place-action">
                                    <a href="#" class="cancel" title="Cancel">Отменить</a>
                                    <a href="{{route('thanks_reservation', $reserve->id)}}" class="detail" title="Detail">Подробнее</a>
                                </td>
                            </tr>
                        @endforeach
