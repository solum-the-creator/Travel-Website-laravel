(function ($) {
    ...
    $(document).ready(function () {
        ...
        $( '.list li' ).on( 'click', function() {
            ...
            $( this ).parents( '.filter-control' ).find( '> span' ).text( label_text );

            $( this ).parents( '.select-box' ).find('select option[value='+label+']').prop("selected", true);
        ...
        $( '.list li' ).on( 'click', function() {
            ...
            if($( this ).parents( '.select-box' ).find('#filter-route').length || $( this ).parents( '.select-box' ).find('#filter-route-category').length){
                let categoryValue = $( this ).parents( '.select-box' ).find('#filter-route-category').val();
                let spotValue = $( this ).parents( '.select-box' ).find('#filter-route').val();

                $.ajax({
                    url: '/filter-route',         /* Куда пойдет запрос */
                    ...
                    success: function(response){
                        ...
                        $('.area-places').empty();
                        response.routes.forEach(routePl=>{

                            let icon = '';
                            if(routePl.categories_id == 1){

                                icon = `<a className="entry-category purple-light" href="#">
                                    <i className="las la-walking"></i><span>Пеший маршрут</span>`;

                                }
                            else if(routePl.categories_id == 2){
                                icon = `<a className="entry-category green-light" href="#">
                                    <i className="las la-biking"></i><span>Велосипедный маршрут</span>`;

                            }
                            else {

                                icon = `<a className="entry-category blue-light" href="#">
                                    <i className="las la-ship"></i><span>Водный маршрут</span>`;
                            ...
                            let placeField = `<div class="place-item layout-02 place-hover" data-maps_name="${routePl.id}">
                                <div class="place-inner">
                                    <div class="place-thumb">
                                        <a class="entry-thumb" href="${route('single-route',routePl.id)}"><img src="/public/storage/${routePl.images}" alt="${routePl.name}}"></a>
                                    ...
                                    <div class="entry-detail">
                                        <div class="entry-head">
                                            <div class="place-type list-item">
                                                <span>${ routePl.category_name}</span>
                                            </div>

                                        </div>
                                        <h3 class="place-title"><a href="${route('single-route',routePl.id)}">${routePl.name}</a></h3>

                                        <div class="entry-bottom">
                                            <div class="place-preview">
                                                <div class="place-rating">
                                                    <span>${routePl.avg_mark}</span>
                                                    <i class="la la-star"></i>
                                                </div>
                                                <span class="count-reviews">(${routePl.count_review}} отзыва)</span>
                                            </div>
            ...
            if($( this ).parents( '.select-box' ).find('#sort-by').length){
                let value = $( this ).parents( '.select-box' ).find('select').val();

                $.ajax({
                    url: '/sort-base-place',         /* Куда пойдет запрос */
                    method: 'get',             /* Метод передачи (post или get) */
                    dataType: 'json',          /* Тип данных в ответе (xml, json, script, html). */
                    data: {
                        "_token": "{{ csrf_token() }}",
                    ...
                    success: function(response){
                        ...
                        response.places.forEach(place=>{
                            ...
                            let placeField = `<div class="place-item layout-02 place-hover" data-maps_name="${place.id}">
                                <div class="place-inner">
                                    <div class="place-thumb">
                                        <a class="entry-thumb" href="${route('single-base-place',place.id)}"><img src="/public/storage/${place.cover_image}" alt="${place.cover_image}"></a>
                                        <a href="#" class="golo-add-to-wishlist btn-add-to-wishlist " data-place-id="185">
											<span class="icon-heart">
												<i class="la la-bookmark large"></i>
											</span>
                                        ...
                                        <a class="entry-category rosy-pink" href="#">
                                            <i class="las la-utensils"></i><span>База отдыха</span>
                                        </a>
                                    </div>
                                    <div class="entry-detail">
                                        <div class="entry-head">
                                        ...
                                        <div class="entry-bottom">
                                            <div class="place-preview">
                                                <div class="place-rating">
                                                    <span>${place.avg_mark}</span>
                                                    <i class="la la-star"></i>
                                                </div>
                                                <span class="count-reviews">(${place.count_review} отзыва)</span>
                                            </div>
                                            <div class="place-price">
                                                <span>от ${place.min_price} BYN</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        // Datepicker
        var disabledDates = [{day: '4', month: '6', year: '2022' }, {day: '6', month: '6', year: '2022' }];
        disabledDates.forEach( function(dayDate) {
            dayDate.month-=1;
        });
        ...
        $('.datepicker').each( function() {
            ...
            $( this ).datetimepicker({
                ...
                onGenerate:function( ct ){
                    ...
                    jQuery(this).find('.xdsoft_date[data-date='+ disabledDates[0].day+'][data-month='+disabledDates[0].month+'][data-year='+ disabledDates[0].year+']')
       ...
       /* jQuery(function(){
            jQuery('#date_timepicker_start').datetimepicker({
                format:'Y/m/d',
                onShow:function( ct ){
                    /!*let dateDisable = $('#date_timepicker_end .xdsoft_date.xdsoft_disabled');
                    console.log(dateDisable.data("date"));
                    console.log(dateDisable.data("month"));
                    console.log(dateDisable.data("year"));*!/
            ...
            jQuery('#date_timepicker_end').datetimepicker({
                ...
                onShow:function( ct ){
                    ...
                    this.setOptions({
                        minDate:jQuery('#date_timepicker_start').val()?jQuery('#date_timepicker_start').val():new Date()
                    })
                },
                timepicker:false
            });
        ...
        $('.datepicker_reserve').each( function() {
            $( this ).datetimepicker({
                /*onGenerate: function(ct, $i){
                    var date=moment(ct).format('d.m.Y');
                    var datesArray='';
                ...
                minDate:'-2060/01/01',//yesterday is minimum date(for today use 0 or -1970/01/01)
                maxDate:'+1952/01/04',//tomorrow is maximum date calendar
                timepicker:false,
                format:'d.m.Y',
       ...
       - - - - - - - - - - - - - - - - - - - - */
        $('.shop-details__quantity').each(function () {
            ...
            if($('.field-type-reservation select').val() == 1){
                idRoom = $('.field-room select').find(":selected").val();
            ...
            plus.on('click', function () {
                /*$.ajax({
                    url: '/get-date-room-form',         /!* Куда пойдет запрос *!/
                    method: 'get',             /!* Метод передачи (post или get) *!/
                    dataType: 'json',          /!* Тип данных в ответе (xml, json, script, html). *!/
                    data: {
                        "_token": "{{ csrf_token() }}",
                ...
                var qty = $(this).parent().find('.qty');
                let maxQty = parseInt(qty.prop('max'),10);
        ...
        $('.field-room select').on('change',function (e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    $('#numberGuest').prop('max',response.numberGuest);
                    if($('.number_adults').val() > parseInt($('#numberGuest').prop('max'),10)){
                        $('.number_adults').val(response.numberGuest);
                        $('.field-guest').find('input[type="text"]').val(response.numberGuest.toString());
                    }
                    ...
                    response.dates.forEach(date=>{
                        ...
                        let reserveDate = new Date(date.reserve_date);

                        dayDate = reserveDate.getDate();
                        monthDate = reserveDate.getMonth();
                        yearDate = reserveDate.getFullYear();

                        let fullDate = {day: dayDate, month: monthDate, year: yearDate };
                        ...
                        $( this ).datetimepicker({
                            onGenerate:function( ct ){
                                ...
                                disabledDates.forEach( date=>{
                                    ...
                                    jQuery(this).find('.xdsoft_date[data-date='+ date.day+'][data-month='+date.month+'][data-year='+ date.year+']')
                                        .addClass('xdsoft_disabled');
                                });
                            },
        ...
        $('.btn-pick').on('click', function (e){
            e.preventDefault();
            if($('.field-house').find(":selected").data('reservation') != 1){
            if($('.field-type-reservation select').val() != 1){
                $('.field-type-reservation select').val(1);

                $('.field-type-reservation select').trigger('change');
            }

            $('.field-room').fadeIn();
            $('#rooms').val($(this).data("room"));
                }
        ...
        var maxGuest;
        $('.field-house select').on('change',function (e) {
          e.preventDefault();
            let typeField = $(this).parents('.field-house').next();
            let selectedHouse = $(this).find(":selected").data('reservation');
          if(selectedHouse == 1 ){
              typeField.fadeOut();
              typeField.find('option:first').prop('selected', true);
              if($('.field-type-reservation').next().is(':visible')){
                  $('.field-type-reservation').next().fadeOut();
              }
          }
          else if(selectedHouse == 2){

                typeField.find('select option[value="2"]').fadeOut();
              typeField.fadeIn();
            }
          else if(selectedHouse == 3){
              typeField.find('select option[value="1"]').fadeIn();
              typeField.find('select option[value="2"]').fadeIn();
              typeField.fadeIn();
          }
          else {
              typeField.fadeIn();
            ...
            let houseId = $('#house').val();
            $.ajax({
                url: '/get-date-house-form',         /* Куда пойдет запрос */
                method: 'get',             /* Метод передачи (post или get) */
                dataType: 'json',          /* Тип данных в ответе (xml, json, script, html). */
                data: {
                    "_token": "{{ csrf_token() }}",
        ...
        $('.field-type-reservation select').on('change',function (e) {
            e.preventDefault();

            if($(this).val() == 1){
                let roomsField = $(this).parents('.field-type-reservation').next();
                roomsField.fadeIn();
                let houseId = $('#house').val();
                $.ajax({
                    url: '/get-rooms-form',         /* Куда пойдет запрос */
                    method: 'get',             /* Метод передачи (post или get) */
                    dataType: 'json',          /* Тип данных в ответе (xml, json, script, html). */
                    data: {
                        "_token": "{{ csrf_token() }}",
                    ...
                    success: function(response){
                        roomsField.find("select").empty();
                        roomsField.find("select").append('<option value="" selected disabled hidden>Номера</option>');
                        response.rooms.forEach(room=>{
                            let roomField = `<option value="${room.id}">${room.name}</option>`;
                            roomsField.find("select").append(roomField);
                ...
                $(this).parents('.field-type-reservation').next().fadeOut();
                $('#numberGuest').prop('max', maxGuest);
        ...
        $('.shop-details__quantity span').on('click', function (e) {
            e.preventDefault();
            var text = [];
            let max_adults = parseInt($('#numberGuest').prop('max'),10);

            if ($('.number_adults').val() > 0 && $('.number_adults').val() <= max_adults) {


                text.push($('.number_adults').val());
            }
